...
const Followers = ({navigation}) => {
    ...
    useEffect(()=>{
        ...
        firebase.firestore().collection("user").doc(user.uid).collection("MyFollowers").onSnapshot((snapshot)=>{
    ...
    const ListEmptyComponent = () => {
        return(
            <View style={{flex:1, justifyContent:"center", alignItems:"center"}}>
                <Text style={{fontSize:17}}>Kimse sizi takip etmiyor!</Text>
            </View>
        )
    }
    ...
    return (
        <View style={{flex:1, backgroundColor:"#fff"}}>
            ...
            <Text style={{fontSize:20, fontWeight:"bold"}}>TAKİPÇİLERİM</Text>
            </View>
            <FlatList 
            data = {users}
